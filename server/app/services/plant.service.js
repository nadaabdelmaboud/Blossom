...
const PlantService = {
    ...
    async getPlant(plant){
      ...
      if (!isValid) return { data: false, err: await error("Invalid Plant ID", 400) };
    ...
    async createPlant(plant){
        ...
        if(!isUnique || isUnique.length !=0) return { data: false, err: await error("This Plant Already Exists", 403) };
